// ---------------------------------------------------------------------------
// Backend URL — set window.BACKEND_URL before this script loads (via
// vercel.json rewrites or a config snippet) OR it falls back to localhost.
// In production this is injected by config.js (see below).
// ---------------------------------------------------------------------------
const API_BASE = (window.BACKEND_URL || 'http://localhost:5000') + '/api';

// Image URL helper — centralised so every file just calls getImageUrl()
function getImageUrl(item) {
  if (item.images && item.images.length > 0) {
    ...
    return (window.BACKEND_URL || 'http://localhost:5000') + img;
  }
  return null;
}
